...
public class ControlFlowStatementsTest
{
                                ...
                                newBlock()
                                        .addStatement(
                                                newAssign().setLeft( var( "x" ) ).setRight( operator( Op.PLUS ).addArgument( var( "x" ) ).addArgument( literal( 1 ) ) )
                                                        .setLabel( "test" ) )
                                        .addStatement( newContinue().setTarget( "test" ) )
                        ) ) );
   ...
   public void testThrowable() throws Exception
   {
       ...
       method.setBody( newThrow().setThrowable( newInstance( UnsupportedOperationException.class ).addArgument( literal( "sorry" ) ) ) );
   ...
   public void testTryCatch() throws Exception
   {
       ...
       method.setBody( newTryCatch()
            .setBody( newInvoke().setMethod( "foo" ).setTarget( var( "x" ) ) )
            .addCatch( declare( NullPointerException.class, "npe" ), newInvoke().setMethod( "printStackTrace" ).setTarget( var( "npe" ) ) )
            .addCatch( declare( IllegalStateException.class, "ise" ), newInvoke().setMethod( "printStackTrace" ).setTarget( var( "ise" ) ) )
